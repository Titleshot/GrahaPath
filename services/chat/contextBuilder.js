...
/**
 * Intent-specific STRICT_USER_DATA — avoids full natal dump on every turn.
 */
async function buildIntentAwareChatContext(chart, intent, { dailyWeather, userPlan, premiumUnlocked } = {}) {
  ...
  switch (intent) {
    case 'small_talk': {
      ...
      return {
        ...
        moodHint: dw
          ? {
              primaryPattern: dw.primaryPattern,
              timingSummary: dw.timingSummary
            }
          : null,
    ...
    case 'relationship_question':
      // Get house lord data if available
      const houseLordData = chart.houseLordData || {};
      const seventhLordPlanet = houseLordData.highlights?.seventhLordPlacement;
      
      return {
        ...base,
        dasha: dashaContext,
        h7: planetsCompactRows(planetsInHouses(chart.planets, [7])),
        h7Lord: seventhLordPlanet ? [{
          name: seventhLordPlanet.lord,
          sign: seventhLordPlanet.sign,
          house: seventhLordPlanet.house,
          dignity: seventhLordPlanet.dignity
        }] : [],
        venusMoon: planetsCompactRows(planetsNamed(chart.planets, ['Venus', 'Moon', 'Mars', 'Jupiter'])),
        venus: planetsCompactRows(planetsNamed(chart.planets, ['Venus'])),
        moon: planetsCompactRows(planetsNamed(chart.planets, ['Moon'])),
        M: chart.moonSign || null,
        L: chart.ascendant || null
      };
...
module.exports = { buildIntentAwareChatContext, currentLiveTransit };
