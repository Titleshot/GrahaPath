/**
 * Lightweight chat intent for orchestration (not full NLP).
 * Order matters: more specific intents before general fallbacks.
 */

function detectChatIntent(rawText) {
  const t = String(rawText || '').trim();
  const lower = t.toLowerCase();

  // Giant broad questions that need narrowing
  if (
    /(tell\s+me\s+everything|tell\s+me\s+all|my\s+whole\s+life|complete\s+life|my\s+entire\s+chart|full\s+life\s+prediction|everything\s+about\s+me|what\s+is\s+my\s+entire|decode\s+my\s+whole|analyze\s+my\s+complete|my\s+full\s+story|all\s+about\s+my\s+life|सम्पूर्ण\s*जीवन|मेरो\s*सम्पूर्ण|सबै\s*कुरा|सबै\s*जानकारी)/i.test(
      t
    )
  ) {
    return 'broad_question_needs_narrowing';
  }
  ...
  if (/(mantra|remedy|upaya|उपाय|puja|पूजा|kasari\s+fix|what\s+should\s+i\s+do\s+spirit|ratna|रत्न|gemstone)/i.test(t)) {
    return 'remedies_question';
  }

  if (
    /(dasha|mahadasha|mahadasha|antardasha|antar dasha|pratyantar|vimshottari|timing|current\s+phase|life\s+phase|what\s+phase\s+am\s+i\s+in|what\s+does\s+my\s+dasha\s+say|why\s+is\s+life\s+changing|why\s+am\s+i\s+feeling\s+pressure|timing\s+am\s+i\s+under)/i.test(
      lower
    )
  ) {
  ...
  if (
    /(why\s+do\s+i|why\s+am\s+i|overthink|over\s+think|emotional|attached|withdraw|pressure|feel\s+anxious|मन\s+अशान्त|चिन्ता)/i.test(
      lower
    )
  ) {
...
module.exports = { detectChatIntent };
